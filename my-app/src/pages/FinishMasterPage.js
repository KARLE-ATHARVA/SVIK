      ...
      onConfirm: async () => {
        try {
          ...
          if (res.data === 'alreadyexists') {
            ...
            alert(`Failed to update finish: ${res.data}`);
          }
        } catch (err) {
                ...
                {filteredFinishes.map((finish) => (
                  <tr key={finish.finish_id}>
                    ...
                    <td className="px-4 py-3">
                      <input
                        type="checkbox"
                        checked={finish.block}
                        onChange={() => confirmBlockToggle(finish)}
                      />
                    </td>
                    <td className="px-4 py-3">{finish.updated_by}</td>
                    <td className="px-4 py-3">
                      {new Date(finish.updated_date).toLocaleDateString()}
                    </td>
                    <td className="px-4 py-3 flex space-x-2">
                      {editId === finish.finish_id ? (
                        <>
                        ...
                        <>
                          ...
                          <button
                            onClick={() => confirmBlockToggle(finish)}
