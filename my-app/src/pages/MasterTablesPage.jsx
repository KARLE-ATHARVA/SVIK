            ...
            <div className="flex-1 overflow-y-auto overflow-x-hidden pr-2 scrollbar-thin scrollbar-thumb-green-500 dark:scrollbar-thumb-green-600 scrollbar-track-gray-200 dark:scrollbar-track-gray-700 rounded-md">
              <ul className="space-y-4">
                {filteredTables.map((table, index) => (
                  <li
                    key={index}
                    className={`text-sm font-light px-4 py-[6px] border-b border-gray-200 dark:border-gray-700
                      ${index % 2 === 0 ? 'bg-white dark:bg-gray-800' : 'bg-gray-50 dark:bg-gray-900'}`}
                  >
                    <Link
                      to={table.path}
                      className="text-green-700 dark:text-green-400 hover:text-green-600 dark:hover:text-green-300 block w-full"
                    >
                      {table.name}
                    </Link>
                  </li>
